#!/usr/bin/env python3
import csv
from pathlib import Path

from PIL import Image, ImageDraw, ImageFont

ROOT = Path("/Serverless/iccd/experiments/20260520-gapbs-full-latest-policy-window-sweep")
SUMMARY = ROOT / "summaries" / "gapbs_full_latest_policy_window_sweep_summary.csv"
PIVOT = ROOT / "summaries" / "gapbs_full_latest_policy_window_compare.csv"
MD = ROOT / "summaries" / "gapbs_full_latest_policy_window_compare.md"
GRAPH_DIR = ROOT / "graphs"
FIG_DIR = Path("/Serverless/iccd/experiments/figure")

WORKLOADS = ("pr", "bc")
CAPS = ("8g", "16g")
WINDOWS = ("5", "10", "20")
BAR_KEYS = ("off", "on", "oneshot", "toggle")
COLORS = {
    "off": "#4d4d4d",
    "on": "#d95f02",
    "oneshot": "#1b9e77",
    "toggle": "#377eb8",
}
LABELS = {
    "off": "off",
    "on": "on",
    "oneshot": "one-shot",
    "toggle": "toggle",
}


def font(size, bold=False):
    paths = [
        "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf" if bold else "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
        "/usr/share/fonts/truetype/liberation2/LiberationSans-Bold.ttf" if bold else "/usr/share/fonts/truetype/liberation2/LiberationSans-Regular.ttf",
    ]
    for path in paths:
        if Path(path).exists():
            return ImageFont.truetype(path, size)
    return ImageFont.load_default()


def text_size(draw, text, fnt):
    box = draw.textbbox((0, 0), text, font=fnt)
    return box[2] - box[0], box[3] - box[1]


def center(draw, x, y, text, fnt, fill="#111111"):
    w, h = text_size(draw, text, fnt)
    draw.text((x - w / 2, y - h / 2), text, font=fnt, fill=fill)


def ms_to_s(value):
    if not value:
        return ""
    return f"{float(value) / 1000.0:.3f}"


def fmt(value):
    if value in ("", None):
        return "n/a"
    return f"{float(value):.5f}"


def read_data():
    raw = list(csv.DictReader(SUMMARY.open()))
    data = {}
    for row in raw:
        key = (row["workload"], row["cap"], row["policy"], row["window_sec"])
        data[key] = row
    return data


def build_pivot(data):
    rows = []
    for workload in WORKLOADS:
        for cap in CAPS:
            off = data[(workload, cap, "off", "")]
            on = data[(workload, cap, "on", "")]
            for window in WINDOWS:
                one = data[(workload, cap, "oneshot", window)]
                tog = data[(workload, cap, "toggle", window)]
                rows.append({
                    "workload": workload,
                    "cap": cap,
                    "window_sec": window,
                    "off_avg_s": off["avg_trial_s"],
                    "on_avg_s": on["avg_trial_s"],
                    "oneshot_avg_s": one["avg_trial_s"],
                    "oneshot_off_s": ms_to_s(one["first_off_ms"]),
                    "oneshot_reason": one["first_off_reason"],
                    "toggle_avg_s": tog["avg_trial_s"],
                    "toggle_off_s": ms_to_s(tog["first_off_ms"]),
                    "toggle_on_s": ms_to_s(tog["first_on_ms"]),
                    "toggle_reason": tog["first_off_reason"],
                })
    fields = [
        "workload", "cap", "window_sec", "off_avg_s", "on_avg_s",
        "oneshot_avg_s", "oneshot_off_s", "oneshot_reason",
        "toggle_avg_s", "toggle_off_s", "toggle_on_s", "toggle_reason",
    ]
    with PIVOT.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(rows)
    return rows


def build_md(rows):
    lines = [
        "# GAPBS full latest-kernel policy/window comparison",
        "",
        "Values are GAPBS `Average Time` in seconds; lower is better.",
        "",
        "- Kernel: `Linux kernel 6.18.0modified #179 SMP PREEMPT_DYNAMIC Wed May 20 03:30:57 UTC 2026`.",
        "- Graph input: `/root/gapbs_graphs/kron_g28.sg`, loaded with `-f`.",
        "- Trials: 8.",
        "- Runtime knobs: MGLRU `0x0007`, scan size 256MB, scan period min 1000ms, fast scan off, hot threshold 0.",
        "- `off`: cgroup node capacity is set, cgroup migration off, cgroup/global demotion off.",
        "- `on`: cgroup migration/demotion on.",
        "- `one-shot`: migration starts on and turns off once.",
        "- `toggle`: migration starts on, turns off, then re-enables after 2 non-matching windows.",
        "",
    ]
    for workload in WORKLOADS:
        lines += [
            f"## {workload.upper()}",
            "",
            "| cap | window | off | on | one-shot | one-shot off s | toggle | toggle off->on s |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
        ]
        for row in [r for r in rows if r["workload"] == workload]:
            transition = f"{row['toggle_off_s']} -> {row['toggle_on_s']}"
            lines.append(
                f"| {row['cap']} | {row['window_sec']} | {fmt(row['off_avg_s'])} | {fmt(row['on_avg_s'])} | "
                f"{fmt(row['oneshot_avg_s'])} | {row['oneshot_off_s']} | {fmt(row['toggle_avg_s'])} | {transition} |"
            )
        lines.append("")
    MD.write_text("\n".join(lines), encoding="ascii")


def bar_value(row, key):
    return float(row[f"{key}_avg_s"])


def plot_workload(workload, rows):
    subset = [r for r in rows if r["workload"] == workload]
    y_max = max(max(bar_value(r, key) for key in BAR_KEYS) for r in subset)
    y_max = max(60.0, ((int(y_max / 10) + 1) * 10.0))

    width, height = 2100, 1280
    margin_l, margin_r, margin_t, margin_b = 145, 70, 145, 210
    gap_panel = 95
    panel_w = (width - margin_l - margin_r - gap_panel) / 2
    plot_h = height - margin_t - margin_b

    img = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(img)
    title_f = font(44, True)
    subtitle_f = font(23)
    axis_f = font(27, True)
    tick_f = font(22)
    value_f = font(17, True)
    small_f = font(15, True)
    legend_f = font(23)

    center(draw, width / 2, 55, f"{workload.upper()} Latest Kernel Policy/Window Sweep", title_f)
    center(draw, width / 2, 100, "Average Time (s), lower is better", subtitle_f, "#555555")

    for ci, cap in enumerate(CAPS):
        x0 = margin_l + ci * (panel_w + gap_panel)
        x1 = x0 + panel_w
        y0 = margin_t + plot_h
        draw.line((x0, margin_t, x0, y0), fill="#111111", width=3)
        draw.line((x0, y0, x1, y0), fill="#111111", width=3)
        center(draw, x0 + panel_w / 2, margin_t - 42, cap.upper(), axis_f)

        tick_step = 10
        for tick in range(0, int(y_max) + 1, tick_step):
            y = y0 - (tick / y_max) * plot_h
            draw.line((x0, y, x1, y), fill="#dddddd", width=1)
            draw.line((x0 - 8, y, x0, y), fill="#111111", width=2)
            tw, th = text_size(draw, str(tick), tick_f)
            draw.text((x0 - tw - 18, y - th / 2), str(tick), font=tick_f, fill="#222222")

        cap_rows = [r for r in subset if r["cap"] == cap]
        group_w = panel_w / len(WINDOWS)
        bar_w = 55
        bar_gap = 8
        inner_w = len(BAR_KEYS) * bar_w + (len(BAR_KEYS) - 1) * bar_gap
        for wi, window in enumerate(WINDOWS):
            row = next(r for r in cap_rows if r["window_sec"] == window)
            gx = x0 + group_w * wi + group_w / 2
            start = gx - inner_w / 2
            for bi, key in enumerate(BAR_KEYS):
                val = bar_value(row, key)
                bx0 = start + bi * (bar_w + bar_gap)
                bx1 = bx0 + bar_w
                by0 = y0 - (val / y_max) * plot_h
                draw.rectangle((bx0, by0, bx1, y0), fill=COLORS[key], outline="#111111", width=2)
                label = f"{val:.1f}"
                lw, lh = text_size(draw, label, value_f)
                draw.text((bx0 + bar_w / 2 - lw / 2, by0 - lh - 7), label, font=value_f, fill="#111111")
                if key == "oneshot":
                    event = f"off@{float(row['oneshot_off_s']):.0f}s"
                elif key == "toggle":
                    event = f"{float(row['toggle_off_s']):.0f}->{float(row['toggle_on_s']):.0f}s"
                else:
                    event = ""
                if event:
                    ew, eh = text_size(draw, event, small_f)
                    draw.text((bx0 + bar_w / 2 - ew / 2, by0 - lh - eh - 10), event, font=small_f, fill="#111111")
            center(draw, gx, y0 + 43, f"{window}s", axis_f)

    center(draw, 45, margin_t + plot_h / 2, "Average Time (s)", axis_f)

    leg_y = height - 105
    total = 0
    items = []
    for key in BAR_KEYS:
        label = LABELS[key]
        tw, th = text_size(draw, label, legend_f)
        item_w = 34 + 12 + tw + 42
        items.append((key, label, tw, th, item_w))
        total += item_w
    lx = width / 2 - total / 2
    for key, label, tw, th, item_w in items:
        draw.rectangle((lx, leg_y - 15, lx + 30, leg_y + 15), fill=COLORS[key], outline="#111111", width=2)
        draw.text((lx + 42, leg_y - th / 2), label, font=legend_f, fill="#111111")
        lx += item_w

    GRAPH_DIR.mkdir(parents=True, exist_ok=True)
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    out = GRAPH_DIR / f"gapbs_full_latest_{workload}_policy_window_compare.png"
    fig = FIG_DIR / f"gapbs_full_latest_{workload}_policy_window_compare.png"
    img.save(out)
    img.save(fig)
    print(out)
    print(fig)


def main():
    data = read_data()
    rows = build_pivot(data)
    build_md(rows)
    for workload in WORKLOADS:
        plot_workload(workload, rows)
    print(PIVOT)
    print(MD)


if __name__ == "__main__":
    main()
