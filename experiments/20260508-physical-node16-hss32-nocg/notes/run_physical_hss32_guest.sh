...
log() {
  printf '[physical-node16-hss32] %s\n' "$*"
}
...
restore_globals() {
  ...
  if [[ -n "${ORIG_LRU_GEN}" ]]; then
    write_optional /sys/kernel/mm/lru_gen/enabled "${ORIG_LRU_GEN}"
  fi
  if [[ -n "${ORIG_SCAN_SIZE}" ]]; then
    write_optional /sys/kernel/debug/sched/numa_balancing/scan_size_mb "${ORIG_SCAN_SIZE}"
  fi
}
...
for node in sorted(nodes):
    ...
    print(f"N{node}_bytes {pages * page_size}")
    print(f"N{node}_gib {pages * page_size / (1024**3):.6f}")
PY
}
...
summarize_run() {
  local out_json="$1"
  python3 - "${CASE_DIR}" "${POLICY_LABEL}" > "${out_json}" <<'PY'
import csv
import json
import math
import statistics
...
if csv_path.exists():
    with csv_path.open("r", encoding="utf-8", errors="replace", newline="") as f:
        ...
        for row in reader:
            rows.append(row)
            try:
                elapsed = int(row.get("time_ms", "0"))
                ops_delta = int(row.get("ops_delta", "0"))
            except ValueError:
                continue
            if elapsed > 0:
                rates.append(float(ops_delta))
...
after_numa = read_numa_summary(case / "numa_maps.after.summary")
summary = {
    "candidate": "skew_lf_hotremote_32g_fixed_rss16g_mulshift_persistent",
    "policy": policy_label,
    "cgroup": "none",
    ...
    "steady_cv": (statistics.pstdev(steady) / statistics.mean(steady)) if len(steady) > 1 and statistics.mean(steady) else 0.0,
    ...
    "final_proc_n0_gib": after_numa.get("N0_gib", 0.0),
    "final_proc_n1_gib": after_numa.get("N1_gib", 0.0),
}
...
snapshot_numa_maps "${bench_pid}" "${CASE_DIR}/numa_maps.after"
summarize_numa_maps "${CASE_DIR}/numa_maps.after" "${CASE_DIR}/numa_maps.after.summary"
