...
WINDOW_SEC="${WINDOW_SEC:?set WINDOW_SEC to 20, 10, or 5}"
EXP_NAME="${EXP_NAME:-20260518-stream32-localutil-window-sweep}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-stream32-localutil-window${WINDOW_SEC}s-300s}"
...
env \
  ...
  BUILD_INITRD=1 \
  INITRD_NAME="initramfs-6.18.0modified-stream32-localutil-window-sweep-20260518.img" \
  MEMORY=96G \
  CPUS=32 \
  HOST_CPUS=0-31 \
