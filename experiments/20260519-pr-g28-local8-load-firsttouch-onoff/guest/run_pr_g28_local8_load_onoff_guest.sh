...
OUTDIR="${OUTDIR:-/tmp/pr_g28_local8}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
CGROOT="${CGROOT:-/sys/fs/cgroup}"
CG_PREFIX="${CG_PREFIX:-prg28}"
...
DROP_CACHES_BETWEEN_POLICIES="${DROP_CACHES_BETWEEN_POLICIES:-1}"
...
run_local_util_controller() {
  ...
  printf 'start,%s,0,0,%s,%s,%s,%s,0,0,0,0,0.00,0.00,0,%s\n' \
    ...
    "${window_seq}" "${window_sec}" "${threshold_pct}" "${min_pte}" \
  ...
  while [[ ! -f "${stop_file}" ]]; do
    ...
    printf 'sample,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s.%02d,%s.%02d,%s,%s\n' \
      ...
      "${window_seq}" "${window_sec}" "${threshold_pct}" "${min_pte}" \
      "${delta_pte}" "${delta_hit}" "${delta_refault}" "${delta_lost}" \
      "${access_whole}" "${access_frac}" "${fast_whole}" "${fast_frac}" \
      "${consecutive}" "${node_balancing}"
...
run_policy() {
  ...
  mkdir -p "${dir}"
  if [[ "${DROP_CACHES_BETWEEN_POLICIES}" == "1" ]]; then
    sync
...

  {
    ...
    echo "drop_caches_before_policy=${DROP_CACHES_BETWEEN_POLICIES}"
  } > "${dir}/run_config.txt"
