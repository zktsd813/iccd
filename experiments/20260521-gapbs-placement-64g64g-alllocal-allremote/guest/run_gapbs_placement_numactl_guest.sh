...
OUTROOT="${OUTROOT:-/root/gapbs-placement-64g64g-numactl}"
...

{
  ...
  echo "placement_baseline=numactl --cpunodebind=0 --membind=<node>"
  echo "policy=off"
...
workload_cmd() {
  ...
  local memnode="$2"
  if [[ "${workload}" == "pr" ]]; then
    printf '%s\0' /usr/bin/numactl --cpunodebind=0 --membind="${memnode}" /root/pr -f "${GRAPH}" -i20 -t1e-4 -n "${TRIALS}"
  else
    printf '%s\0' /usr/bin/numactl --cpunodebind=0 --membind="${memnode}" /root/bc -f "${GRAPH}" -i1 -n "${TRIALS}"
  fi
}
...
run_case() {
  ...
  mkdir -p "${outdir}"
  mapfile -d '' -t cmd < <(workload_cmd "${workload}" "${memnode}")

  log "start workload=${workload} placement=${placement} membind=${memnode}"
  sync || true
  ...
  "${RUNNER}" \
    ...
    --cgroup-name "gapbs_numactl_${workload}_${placement}_$$" \
    --capacity-node 0 \
    --capacity-pages 0 \
    --node-balancing 0 \
    --scan-size-mb 256 \
...
def numa_field(path, field, node):
    if path.exists():
        for line in path.read_text(errors="replace").splitlines():
...
for workload_dir in sorted(p for p in root.iterdir() if p.is_dir()):
    ...
    for case_dir in sorted(p for p in workload_dir.iterdir() if p.is_dir()):
        ...
        row = {
            ...
            "file_n0_after": numa_field(case_dir / "cgroup.after", "file", 0),
            "file_n1_after": numa_field(case_dir / "cgroup.after", "file", 1),
            "anon_n0_after": numa_field(case_dir / "cgroup.after", "anon", 0),
            "anon_n1_after": numa_field(case_dir / "cgroup.after", "anon", 1),
        }
        for idx in range(8):
...
fieldnames = [
    ...
    "avg_trial_s", "numa_hint_faults", "pgpromote_success",
    "pgdemote_kswapd", "pgdemote_direct", "file_n0_after",
    "file_n1_after", "anon_n0_after", "anon_n1_after",
