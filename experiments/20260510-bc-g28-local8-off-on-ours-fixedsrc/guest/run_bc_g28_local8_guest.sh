...
BC_BIN="${MBENCH:-/root/bc}"
OUTDIR="${OUTDIR:-/tmp/bc_g28_local8}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
CGROOT="${CGROOT:-/sys/fs/cgroup}"
CG_PREFIX="${CG_PREFIX:-bcg28}"
...
CAPACITY_PAGES="${CAPACITY_PAGES:-2097152}" # 8 GiB / 4 KiB

BC_SCALE="${BC_SCALE:-28}"
BC_ITERS="${BC_ITERS:-1}"
BC_TRIALS="${BC_TRIALS:-10}"
BC_SOURCE="${BC_SOURCE:-0}"
BC_LOGGING="${BC_LOGGING:-0}"
GAPBS_PREBUILD_GRAPH="${GAPBS_PREBUILD_GRAPH:-1}"
GAPBS_GRAPH_FILE="${GAPBS_GRAPH_FILE:-/root/gapbs_graphs/kron_g${BC_SCALE}.sg}"
...
if [[ ! -x "${BC_BIN}" ]]; then
  echo "BC binary not executable: ${BC_BIN}" >&2
  exit 1
fi

log() {
  printf '[bc-g28-guest] %s\n' "$*"
}

prepare_gapbs_graph() {
  GAPBS_INPUT_OPT="-g"
  GAPBS_INPUT_VALUE="${BC_SCALE}"
  GAPBS_INPUT_DESC="-g${BC_SCALE}"
  ...
  if [[ -s "${GAPBS_GRAPH_FILE}" ]]; then
    ...
    log "building cached graph ${GAPBS_GRAPH_FILE} from -g${BC_SCALE}"
    rm -f "${tmp}"
    env OMP_NUM_THREADS="${OMP_THREADS}" OMP_PROC_BIND=true OMP_PLACES=cores \
      "${GAPBS_CONVERTER}" -g"${BC_SCALE}" -b "${tmp}" \
...
run_policy() {
  local policy="$1"
  local cg="${CGROOT}/${CG_PREFIX}_${policy}_${RUN_ID}"
  local dir="${RUN_ROOT}/${policy}"
  local stop_file sampler_pid controller_pid start_s end_s rc bc_args cmd_for_log

  mkdir -p "${dir}"
  log "policy=${policy} setup cgroup=${cg}"
  setup_cgroup "${cg}" "${policy}"

  bc_args="'${GAPBS_INPUT_OPT}' '${GAPBS_INPUT_VALUE}' -i'${BC_ITERS}' -n'${BC_TRIALS}'"
  cmd_for_log="${BC_BIN} ${GAPBS_INPUT_DESC} -i${BC_ITERS} -n${BC_TRIALS}"
  if [[ -n "${BC_SOURCE}" && "${BC_SOURCE}" != "rand" ]]; then
    bc_args="${bc_args} -r'${BC_SOURCE}'"
    cmd_for_log="${cmd_for_log} -r${BC_SOURCE}"
  fi
  if [[ "${BC_LOGGING}" == "1" ]]; then
    bc_args="${bc_args} -l"
    cmd_for_log="${cmd_for_log} -l"
  fi

  {
    echo "policy=${policy}"
    echo "command=${cmd_for_log}"
    echo "source=${BC_SOURCE}"
    echo "logging=${BC_LOGGING}"
  ...
  timeout "${TIMEOUT_SEC}" bash -c \
    "echo \$\$ > '${cg}/cgroup.procs'; exec env OMP_NUM_THREADS='${OMP_THREADS}' OMP_PROC_BIND=true OMP_PLACES=cores '${BC_BIN}' ${bc_args}" \
...
log "uname=$(uname -a)"
log "BC=$(${BC_BIN} -h 2>&1 | head -1 || true)"
prepare_gapbs_graph
