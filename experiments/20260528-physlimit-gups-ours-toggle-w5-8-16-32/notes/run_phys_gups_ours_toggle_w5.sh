...
EXP=/Serverless/iccd/experiments/20260528-physlimit-gups-ours-toggle-w5-8-16-32
...
run_one() {
  ...
  local name="togw5-${cap_gb}g"
  local host_dir="${EXP}/guest-results/${cap_gb}g"
  local note_dir="${EXP}/notes/${cap_gb}g"
  local guest_dir="/root/physlimit-gups-ours-toggle-w5/${cap_gb}g"
  ...
  "${SSH_BASE[@]}" -p "${port}" root@127.0.0.1 'mkdir -p /root/scripts /root/benchmark/vmitosis-workloads/bin /root/physlimit-gups-ours-toggle-w5' >/dev/null
  ...
  "${SSH_BASE[@]}" -p "${port}" root@127.0.0.1 \
    "/root/scripts/run_local_util_adapt_experiment.sh \
      --outdir ${guest_dir}/toggle_w5 \
      --run-id gups_phys${cap_gb}g_toggle_w5 \
      --policy ours \
      --capacity-pages 0 \
  ...
  rm -rf "${host_dir}/toggle_w5"
  "${SCP_BASE[@]}" -P "${port}" -r root@127.0.0.1:"${guest_dir}/toggle_w5" "${host_dir}/" >/dev/null
