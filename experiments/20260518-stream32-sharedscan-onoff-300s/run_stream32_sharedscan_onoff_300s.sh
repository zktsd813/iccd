...
EXP_NAME="${EXP_NAME:-20260518-stream32-sharedscan-onoff-300s}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-stream32-sharedscan-onoff-300s}"
...
env \
  ...
  BUILD_INITRD=1 \
  INITRD_NAME="initramfs-6.18.0modified-stream32-sharedscan-20260518.img" \
  MEMORY=96G \
  CPUS=32 \
  HOST_CPUS=0-31 \
