...
static vm_fault_t do_numa_page(struct vm_fault *vmf)
{
	...
	if (folio_test_local_tiering_sampled(folio) &&
	    task_numa_balancing_mode(current) <= 0) {
		numa_migrate_check(folio, vmf, vmf->address, &flags,
				   writable, &last_cpupid, NULL);
		nid = NUMA_NO_NODE;
		goto out_map;
	}
