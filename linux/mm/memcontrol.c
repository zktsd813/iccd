...
static void memcg_reclaimd_maybe_wake_on_charge(struct mem_cgroup *memcg,
						...
						gfp_t gfp_mask);
#else
static inline void memcg_reclaimd_init(struct mem_cgroup *memcg) { }
static inline int memcg_reclaimd_start(struct mem_cgroup *memcg) { return 0; }
static inline void memcg_reclaimd_stop(struct mem_cgroup *memcg) { }
static inline void memcg_numa_balancing_disable(struct mem_cgroup *memcg) { }
static inline void memcg_node_exact_account(struct mem_cgroup *memcg, int nid,
					    long nr_pages) { }
static inline void memcg_node_exact_sync(struct mem_cgroup *memcg) { }
static inline void memcg_reclaimd_maybe_wake_on_charge(struct mem_cgroup *memcg,
						       struct folio *folio,
						       gfp_t gfp_mask) { }
#endif
...
static struct mem_cgroup *mem_cgroup_alloc(struct mem_cgroup *parent)
{
	...
	atomic64_set(&memcg->numa_migrate_success_total, 0);
	atomic64_set(&memcg->numa_migrate_success_promotion, 0);
	atomic64_set(&memcg->numa_migrate_success_other, 0);
	atomic64_set(&memcg->numa_migrate_fail_promotion_over_high, 0);
	atomic64_set(&memcg->numa_migrate_fail_promotion_alloc, 0);
...
#ifdef CONFIG_NUMA_BALANCING_MT
#define MEMCG_RECLAIMD_COOLDOWN_JIFFIES		msecs_to_jiffies(1000)
#define MEMCG_RECLAIMD_MAX_FAILURES		3

static bool memcg_node_capacity_enabled(struct mem_cgroup *memcg)
{
...
{
	return memcg_node_capacity_enabled(memcg) &&
	       memcg_kswapd_demotion_active(memcg);
}
...
static u64 memcg_node_high_wmark(struct mem_cgroup_per_node *pn)
{
	...
	if (!cap)
		return 0;
	if (!high)
		high = memcg_node_default_high_wmark(cap);
	if (high > cap)
		high = cap;
	if (high < low)
		high = low;
					  ...
					  u64 high_wmark, u64 low_wmark)
{
	...
	WRITE_ONCE(pn->node_over_high, pages >= high_wmark);
	if (pages < high_wmark)
		WRITE_ONCE(pn->node_next_check, high_wmark - pages);
	else if (low_wmark && pages > low_wmark)
		WRITE_ONCE(pn->node_next_check, pages - low_wmark);
	else
		WRITE_ONCE(pn->node_next_check, 0);
}
...
static void memcg_reclaimd_reset_backoff(struct mem_cgroup_per_node *pn)
{
	WRITE_ONCE(pn->node_reclaimd_failures, 0);
	WRITE_ONCE(pn->node_reclaimd_cooldown_until, 0);
}

static bool memcg_reclaimd_backoff_active(struct mem_cgroup_per_node *pn)
{
...
void memcg_reclaimd_wake_node(struct mem_cgroup *memcg, int nid)
{
	...
	if (!pn || !READ_ONCE(pn->node_capacity) ||
	    memcg_reclaimd_backoff_active(pn))
		return;

	ctx = &memcg->reclaimd;
	spin_lock(&ctx->lock);
	if (!node_isset(nid, ctx->pending_nodes) &&
...
static void memcg_reclaimd_finish_node(struct mem_cgroup *memcg, int nid)
{
	struct memcg_reclaimd_ctx *ctx = &memcg->reclaimd;
				...
				struct memcg_reclaimd_result *result)
{
	...
	pages = mem_cgroup_node_usage_pages_live(memcg, nid);
	...
	nr_to_reclaim = max_t(unsigned long, pages - low_wmark, SWAP_CLUSTER_MAX);
	result->nr_reclaimed =
		try_to_free_mem_cgroup_node_pages(memcg, nid, nr_to_reclaim,
						  GFP_KERNEL,
						  MEMCG_RECLAIM_MAY_SWAP, NULL);
	result->nr_scanned = nr_to_reclaim;

	pages = mem_cgroup_node_usage_pages_live(memcg, nid);
	memcg_node_update_check_state(pn, pages, high_wmark, low_wmark);
	result->balanced = pages <= low_wmark;
	return true;
}

static bool memcg_reclaimd_should_run(struct mem_cgroup *memcg)
{
	struct memcg_reclaimd_ctx *ctx = &memcg->reclaimd;

	return ctx->stop || !nodes_empty(ctx->pending_nodes);
}

static int memcg_reclaimd(void *data)
{
	...
	while (!kthread_should_stop()) {
		wait_event_freezable(ctx->waitq,
				     memcg_reclaimd_should_run(memcg));
		if (ctx->stop)
			break;

		while (!nodes_empty(ctx->pending_nodes) && !ctx->stop) {
			struct memcg_reclaimd_result result;
			struct mem_cgroup_per_node *pn;
			int nid = memcg_reclaimd_pick_node(memcg);
			...
			if (pn) {
				if (result.balanced || result.nr_reclaimed) {
					memcg_reclaimd_reset_backoff(pn);
				} else if (result.nr_scanned) {
					u32 failures = READ_ONCE(pn->node_reclaimd_failures) + 1;

					WRITE_ONCE(pn->node_reclaimd_failures, failures);
					if (failures >= MEMCG_RECLAIMD_MAX_FAILURES)
						WRITE_ONCE(pn->node_reclaimd_cooldown_until,
							   jiffies + MEMCG_RECLAIMD_COOLDOWN_JIFFIES);
				}
			}
			memcg_reclaimd_finish_node(memcg, nid);
			cond_resched();
		}
	}
...
static int memcg_reclaimd_start(struct mem_cgroup *memcg)
{
	...
	if (mem_cgroup_is_root(memcg) || ctx->task)
		return 0;
						...
						gfp_t gfp_mask)
{
	...
	if (high_wmark && pages >= high_wmark)
			       ...
			       unsigned long nr_pages)
{
	...
	if (!memcg_node_capacity_enabled(memcg))
		return false;
	if (nid < 0 || nid >= nr_node_ids || !node_state(nid, N_MEMORY))
		return false;
	...
	pages = mem_cgroup_node_usage_pages_live(memcg, nid);
	projected = min_t(u64, (u64)pages + nr_pages, ULONG_MAX);
	low_wmark = memcg_node_low_wmark(pn);
	memcg_node_update_check_state(pn, projected, high_wmark, low_wmark);
	if (projected < high_wmark)
		return false;
				       ...
				       unsigned long nr_pages)
{
	...
	return mem_cgroup_node_over_high(memcg, nid, nr_pages) ? 0 : 1;
}
			      ...
			      unsigned long nr_pages)
{
	...
	if (allowed && !node_isset(preferred_nid, *allowed))
		goto out;
	if (!mem_cgroup_node_over_high(memcg, preferred_nid, nr_pages))
		goto out;
	...
	for_each_node_state(nid, N_MEMORY) {
		...
		if (allowed && !node_isset(nid, *allowed))
			continue;
		if (mem_cgroup_node_over_high(memcg, nid, nr_pages))
			continue;
		preferred_nid = nid;
		break;
	}
...
static void memcg_numa_balancing_disable(struct mem_cgroup *memcg)
{
	...
	old_mode = xchg(&memcg->node_balancing_mode, 0);
	if (old_mode > 0) {
		atomic_dec(&numa_balancing_count);
		sched_numa_balancing_update_state();
	}
	WRITE_ONCE(memcg->kswapd_demotion_enabled,
		   MEMCG_KSWAPD_DEMOTION_DISABLED);
}
...
static void uncharge_folio(struct folio *folio, struct uncharge_gather *ug)
{
	...
	if (folio_test_clear_local_tiering_sampled(folio))
		mem_cgroup_numa_account_local_fault_lost(memcg, nr_pages);
#endif
...
static int memory_node_capacity_show(struct seq_file *m, void *v)
{
	...
	for_each_node_state(nid, N_MEMORY) {
		...
		seq_printf(m, "N%d=%llu low=%llu high=%llu usage=%lu\n",
			   nid, READ_ONCE(pn->node_capacity),
			   memcg_node_low_wmark(pn),
			   memcg_node_high_wmark(pn),
			   mem_cgroup_node_usage_pages_live(memcg, nid));
	}
					  ...
					  char *buf, size_t nbytes, loff_t off)
{
	...
	WRITE_ONCE(pn->node_low_wmark,
		   capacity ? memcg_node_default_low_wmark(capacity) : 0);
	WRITE_ONCE(pn->node_high_wmark,
		   capacity ? memcg_node_default_high_wmark(capacity) : 0);
	memcg_reclaimd_reset_backoff(pn);
	memcg_node_exact_sync(memcg);
	if (capacity && mem_cgroup_node_over_high(memcg, nid, 0))
		memcg_reclaimd_wake_node(memcg, nid);
					   ...
					   char *buf, size_t nbytes, loff_t off)
{
	...
	if (kstrtoint(strstrip(buf), 0, &mode))
		return -EINVAL;
	if (mode < 0 ||
	...
	struct kernfs_open_file *of, char *buf, size_t nbytes, loff_t off)
{
	...
	if (kstrtouint(strstrip(buf), 0, &rate))
		return -EINVAL;
	if (rate > MEMCG_NUMA_LOCAL_FAULT_RATE_MAX)
		return -EINVAL;
	...
	struct kernfs_open_file *of, char *buf, size_t nbytes, loff_t off)
{
	...
	if (kstrtouint(strstrip(buf), 0, &hit_ms))
		return -EINVAL;
	...
	struct kernfs_open_file *of, char *buf, size_t nbytes, loff_t off)
{
	...
	if (kstrtouint(strstrip(buf), 0, &advance))
		return -EINVAL;
	if (!advance)
		return -EINVAL;
	if (mem_cgroup_is_root(memcg))
...
static int memory_kswapd_demotion_enabled_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%u\n", READ_ONCE(memcg->kswapd_demotion_enabled));
	return 0;
}
						    ...
						    loff_t off)
{
	...
	if (kstrtouint(strstrip(buf), 0, &enable))
		return -EINVAL;
	if (enable > 1)
		return -EINVAL;
	...
	memcg_node_exact_sync(memcg);
	for_each_node_state(nid, N_MEMORY) {
		memcg_reclaimd_reset_backoff(memcg->nodeinfo[nid]);
		if (enable && mem_cgroup_node_over_high(memcg, nid, 0))
			memcg_reclaimd_wake_node(memcg, nid);
	}
...
static struct cftype memory_mt_files[] = {
	...
	{
		.name = "numa_local_fault_state",
		.flags = CFTYPE_NOT_ON_ROOT | CFTYPE_NS_DELEGATABLE,
		.seq_show = memory_numa_local_fault_state_show,
	},
	{
...
static int __init mem_cgroup_mt_init(void)
{
	if (mem_cgroup_disabled())
		return 0;
...
subsys_initcall(mem_cgroup_mt_init);
#endif /* CONFIG_NUMA_BALANCING_MT */
