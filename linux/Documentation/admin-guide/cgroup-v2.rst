...
The following nested keys are defined.
  ...
  memory.numa_balancing_fast_scan
	A read-write single value file which is available on kernels
	built with ``CONFIG_NUMA_BALANCING_MT``.

	This file enables a per-memcg fast NUMA scan diagnostic mode.
	The value is ``0`` by default.

	When set to ``1``, task NUMA balancing scan cadence is still
	controlled by the global NUMA scan period knobs, but the
	``MAX_SCAN_WINDOW`` minimum-period floor and the per-task NUMA
	PTE scanner overhead limiter are bypassed for tasks in the
	cgroup.
  ...
  memory.numa_balancing_hot_threshold_ms
	...
	This file controls the per-memcg hotness threshold used for
	hint-fault driven promotion decisions from a lower tier's
	active list to a higher tier's inactive list.

	The value is expressed in milliseconds of access latency.
	Writing ``0`` keeps the existing global / adaptive threshold
	selection.  Any non-zero value overrides that behavior for
	the target memcg.
  ...
  memory.kswapd_demotion_enabled
	...
	This file controls whether memcg node-capacity pressure is active once
	``memory.node_capacity`` and the associated node-local watermarks
	are exceeded.  The implementation supports one active mode:
	``MEMCG_KSWAPD_DEMOTION_NODE_WMARK``.  Any non-zero value written by
	userspace is normalized to mode ``1``.

	The following values are defined.

	  ``0``
		Disable memcg node-capacity reclaim behavior.

	  ``1``
		Enable exact node-watermark mode.  This mode uses the exact
		per-node charged-page counter for watermark decisions.
		Placement steering keeps the cheaper check-budget shortcuts,
		the charge path avoids synchronous direct reclaim, and the
		background memcg reclaim worker wakes when the high watermark
		is reached.  Once woken, the worker reclaims the memcg's
		node-local LRU down to the low watermark.  This matches
		physical-node watermark hysteresis while avoiding the
		allocator-side cost of checking and reclaiming on every charge.

	Implementation notes:

	- Mode ``1`` maintains an exact per-memcg, per-node
	  charged-page counter.  The counter is updated when a folio is
	  charged, uncharged, or migrated between nodes, and is synchronized
	  from the memcg LRU stats when the mode is enabled.

	- Mode ``1`` uses that exact counter for watermark decisions but
	  keeps budgeted placement checks and background reclaimd hysteresis:
	  high watermark wakes reclaimd, low watermark is the reclaim target.
	  Once reclaimd is woken, it keeps the node in-flight while reclaiming
	  toward the target instead of finishing each short reclaim pass with a
	  fresh queue/wake cycle.  When reclaimd observes a balanced node, it
	  performs a short settle interval before clearing the node so transient
	  post-reclaim charge bursts are absorbed without immediately rebuilding
	  wake storms.

	Implementation hook details:

	- The exact counter is stored in each ``mem_cgroup_per_node`` and is
	  updated from the folio charge, uncharge, and migration paths.  When a
	  cgroup enters mode ``1`` from a disabled state, the
	  counter is bootstrapped from the flushed memcg LRU state before new
	  charge deltas are applied.

	- Placement steering runs through ``mem_cgroup_preferred_node()``.  Mode
	  ``1`` keeps the ``node_next_check`` budget so repeated allocations can
	  skip the full watermark read while the node is still known to be under
	  the high watermark.

	- Charge-time wakeups queue the memcg reclaim worker when the charged
	  node reaches the high watermark.

	- Charge-time direct reclaim is skipped by mode ``1`` to preserve the
	  background-reclaim hysteresis from the validated node-watermark design.

	- The vmscan reclaim target is the low watermark after the worker is
	  woken at high.

  memory.reclaimd_state
	A read-only flat-keyed file which is available on kernels
	built with ``CONFIG_NUMA_BALANCING_MT``.

	This file exposes the state of the memcg reclaim helper used
	by the tiering implementation.

	The following entries are defined.

	  running
		Whether the reclaim helper is currently running.

	  wake_count
		Number of times the helper has been woken.

	  run_count
		Number of completed helper runs.

	  pending_nodes
		Nodemask of nodes queued for reclaim work.

	  inflight_nodes
		Nodemask of nodes currently being processed.

	  nodeX_failures
		For each reclaim-capable node ``X``, the current no-progress
		reclaim failure streak observed by reclaimd while the node is
		in-flight.

	  nodeX_cooldown_ms
		For each reclaim-capable node ``X``, the remaining cooldown time
		before reclaimd is willing to queue a new reclaim pass after the
		failure streak reached its retry limit.

  memory.numa_migrate_state
	A read-only flat-keyed file which is available on kernels
	built with ``CONFIG_NUMA_BALANCING_MT``.

	This file exposes per-memcg NUMA migration success counters.

	The following entries are defined.

	  numa_migrate_success_total
		Total number of successful NUMA migrations charged to the memcg.

	  numa_migrate_success_promotion
		Successful migrations that promoted memory to a higher tier.

	  numa_migrate_success_other
		Successful migrations which were not counted as promotions.
